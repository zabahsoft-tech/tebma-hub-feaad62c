...
import { VideoBadge } from "@/components/site/MediaGallery";
...
export const Route = createFileRoute("/dictionary/")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:url", content: "https://tebma-hub.lovable.app/dictionary" },
    ],
    links: [{ rel: "canonical", href: "https://tebma-hub.lovable.app/dictionary" }],
  }),
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(
      (d) =>
        ...
        plain(d.description).toLowerCase().includes(t) ||
              ...
              <Link key={d.id} to="/dictionary/$slug" params={{ slug: d.slug }} className="group border border-border rounded-md overflow-hidden hover:border-foreground/40 transition-colors">
                ...
                <div className="p-5">
                  <div className="flex items-center gap-2">
                    ...
                    {"has_video" in d && d.has_video ? <VideoBadge /> : null}
                  </div>
                  <p className="mt-2 text-sm text-muted-foreground line-clamp-2">{plain(d.description)}</p>
                  {d.tags?.length ? (
