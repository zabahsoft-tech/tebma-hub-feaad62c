...
export const Route = createFileRoute("/verify/")({
  head: () => ({
    meta: [
      ...
      { property: "og:url", content: "https://tebma-hub.lovable.app/verify" },
    ],
    links: [{ rel: "canonical", href: "https://tebma-hub.lovable.app/verify" }],
  }),
...
function QrScanner({ onResult, onClose }: { onResult: (t: string) => void; onClose: () => void }) {
  ...
  return (
    <div className="border border-border rounded-md p-4 relative">
      <button onClick={onClose} aria-label="Close scanner" className="absolute top-2 right-2 p-1 rounded-sm hover:bg-accent">
        <X className="size-4" />
      </button>
